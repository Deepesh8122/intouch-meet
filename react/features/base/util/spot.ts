...
/**
 * Checks if InTouch Meet is running on Spot TV.
 *
 * @param {IReduxState} state - The redux state.
 * @returns {boolean} Whether or not InTouch Meet is running on Spot TV.
 */
export function isSpotTV(state: IReduxState): boolean {
    const { defaultLocalDisplayName, iAmSpot } = state['features/base/config'] || {};

    return iAmSpot
        || navigator.userAgent.includes('JitsiSpot/') // InTouch Spot app
